...
export default function DashboardPage({
  ...
  const { data: status } = useSWR<ValidationStatus>(`status-${documentId}`, () => getValidationStatus(documentId), {
    refreshInterval: (data) => (data?.status === "complete" || data?.status === "error" ? 0 : 2000),
    fallbackData: { document_id: documentId, status: "complete" },
    onError: () => {
  ...
  const { data: results, mutate: refreshResults } = useSWR<ValidationResults>(
    status?.status === "complete" ? `results-${documentId}` : null,
  ...
  const isLoading = status?.status !== "complete" && status?.status !== "error"
