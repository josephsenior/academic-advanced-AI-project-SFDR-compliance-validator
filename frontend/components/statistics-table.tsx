...
import { Table, BarChart3, Hash, CheckCheck, TrendingUp } from "lucide-react"
...
export function StatisticsTable({ statistics }: StatisticsTableProps) {
  const accuracyPercentage = statistics.numerical_values_checked && statistics.values_matching_reference
    ? Math.round((statistics.values_matching_reference / statistics.numerical_values_checked) * 100)
    : null
  ...
  const stats = [
    ...
    ...(statistics.values_matching_reference ? [{
      label: "Data Accuracy",
      value: `${accuracyPercentage}%`,
      detail: `${statistics.values_matching_reference}/${statistics.numerical_values_checked}`,
      icon: TrendingUp,
    }] : []),
  ]
          ...
          {stats.map((stat) => (
            <div key={stat.label} className="flex items-center gap-3 p-3 rounded-lg bg-muted/50">
              ...
              <div>
                ...
                {'detail' in stat && <p className="text-xs text-muted-foreground">{stat.detail}</p>}
