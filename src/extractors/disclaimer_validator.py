...
class DisclaimerValidator:
    ...
    def __init__(
        ...
        """
        Initialize disclaimer validator.
    ...
    def _detect_present_disclaimers(
        ...
        """Detect which disclaimers are actually present in the document"""
        present = set()
        ...
        for category in all_categories:
            if category in category_to_type:
                present.add(category_to_type[category])
        
        # Check for OBAM presentation (should be on all presentations)
        full_text = (extraction_result.get('full_text', '') or extraction_result.get('text', '')).lower()
        if 'oddo bhf asset management' in full_text or 'obam' in full_text:
            present.add('obam_presentation')
        
        # Check for commercial documentation
        if 'commercial documentation' in full_text or 'documentation commerciale' in full_text:
            present.add('commercial_documentation')
        
        # Check for RAIF
        if 'raif' in full_text or 'luxembourg funds-raif' in full_text:
            present.add('commercial_documentation_raif')
    ...
    def validate(
        ...
        present = self._detect_present_disclaimers(extraction_result)
