...
from .data_consistency_agent import DataConsistencyResult
...
class DocumentCorrector:
    ...
    def correct(
        ...
        if not original_path.exists():
            result.error_message = f"File not found: {original_path}"
            return result
        ...
        if output_path is None:
            ...
            output_path = original_path.parent / f"{stem}_corrected{suffix}"
        else:
        ...
        try:
            ...
            corrector_func(original_path, output_path, validation_result, disclaimer_result, result, auto_fix_disclaimers)
            
            result.corrected_path = str(output_path)
            result.success = True
